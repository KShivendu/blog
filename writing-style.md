...
- `<details>` sections are also where I proofread least, so that's exactly where old AI-draft leftovers hide longest if I'm not careful:
  ...
  - **Banned opener: "Here's the part that surprised me" / "Here's the thing" / "Here's what's interesting".** A drumroll that delays the actual point by a whole sentence. I do say when a result surprised me (see Tone), but I say it about the result itself, "I expected X and got Y", not as a setup phrase before it. Cut the run-up and lead with the finding. Note the exception: **"Now the interesting part." / "Now the fun part." is mine and stays.** That one is a section transition telling you where you are, it doesn't promise a payoff and then withhold it for a sentence.
  - **Banned metaphor: "rung" / "rungs" / "a rung on the ladder".** Same problem as the knob, a ladder image forced onto a list of tools. Name the thing itself: "what each one buys me, and where it stops", or just write the list.
