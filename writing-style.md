...
- `<details>` sections are also where I proofread least, so that's exactly where old AI-draft leftovers hide longest if I'm not careful:
  ...
  - **Banned opener: "Here's the part that surprised me" / "Here's the thing" / "Here's what's interesting" / "Now the interesting part".** A drumroll that delays the actual point by a whole sentence. I do say when a result surprised me (see Tone), but I say it about the result itself, "I expected X and got Y", not as a setup phrase before it. Cut the run-up and lead with the finding.
