...
<BarChart
  ...
  categories={['BPE + gpt2 regex', 'ISBPE single-flag', 'ISBPE dual-flag']}
  series={[
    { name: 'English (C4)', color: '#94a3b8', values: [2.677, 2.685, 2.7] },
    { name: 'Python code', color: '#0d9488', values: [2.428, 2.611, 2.655] },
  ]}
/>
...
## The bug that made the best idea look like the worst

My first measurement of dual-flag came back at **-30.69% on code**. I nearly wrote it off.

The tokenizations looked wrong in a specific way. Whitespace runs that single-flag kept whole (`#········`, eight spaces) were shattering into four separate two-space tokens, and mean bytes per token had collapsed from **4.000 to 2.489**. That's not a scheme performing badly, that's a scheme not training.

The cause was one line in the merge loop. BPE training keeps a heap of candidate pairs with lazy deletion, and I re-pushed a pair only when it was **new**:

```python
if (x, y) not in where or wi not in where[(x, y)]:
    heapq.heappush(heap, (-pairs[(x, y)], x, y))     # wrong
```

Lazy deletion pops an entry only when its stored count still matches the live count. So a pair whose count **grows** without being re-pushed gets rejected forever and can never win a merge. That starves the process of exactly the pairs that are becoming frequent. The fix is to push on every count change:

```python
pairs[(a, b)] += c
where[(a, b)].add(wi)
heapq.heappush(heap, (-pairs[(a, b)], a, b))          # every time
```

The **-30.69%** and the **+9.37%** are the same idea. The only difference was that line. I keep the story because the failure mode was legible if I'd looked at the tokenizations before the aggregate, and I didn't.
