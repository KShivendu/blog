...
And it is instinct for latency. I timed this BM25 baseline at **3.5ms p50, 7.2ms p90**. Reporting only the 4.4ms mean would be a mistake, because the p90 is the request a real user is waiting on. p10 relevance is that same user, on the tail that actually hurts. We just forget to report it.
