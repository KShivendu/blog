...
The median Δ is **0** on all three metrics. However let's look at the reality:

- `who sang here i go again` went from a perfect **1.0 to 0** on NDCG@10. "Here I Go Again" is a song title made of stopwords, so the stoplist deletes it and we only ever query the index for `sang go`.
- `who appoints the members of the given branch in the united states` also went **1.0 to 0**. The stemmer cuts `united states` down to `unit state`, so unrelated stuff gets ranked too.
...
- **9 queries** dropped from a real NDCG@10 score to exactly 0 like that, while NDCG@10 overall shows **172 wins against 133 losses** and a p10 Δ of **−11.7**.
- recall@100 is the most #[forgiving](it only asks whether a relevant doc landed anywhere in the top 100, so a doc can slip 90 places and still count) metric and still shows **84 wins against 32 losses**. On 4 of those the score fell to **0**, so every relevant doc was pushed past rank 100. One asks which state generates the most solar power, and the stemmer cuts "generates" down to #[gener](Porter gives that same stem to general, generally, generic, generous and generation), a stem it now shares with half the dictionary.
- Queries swing the other way too, 0 → 100 (hover a bar to see which).

Percentiles did the work in the first half of this post. Against a change they go flat:

| NDCG@10 | word | word_en | Δ of the percentile | percentile of the Δ |
| ------- | ---: | ------: | ------------------: | ------------------: |
| mean    | 52.5 |    54.5 |                +1.9 |                +1.9 |
| p50     | 55.4 |    59.2 |                +3.7 |            **+0.0** |
| p25     | 21.9 |    26.4 |                +4.5 |                +0.0 |
| p10     |  0.0 |     0.0 |                +0.0 |               −11.7 |

The mean is the only row where those two columns agree, since the mean of the differences is the difference of the means. Every other statistic answers a different question depending on which one I compute.

294 of the 599 queries never moved, so every percentile in the middle of the Δ distribution is 0 by construction. On recall@100, where 483 never move, even p10 comes out at 0, and the mean is the only summary left with a number in it.

Zero-rate still moves, 19.9% of queries scored 0 before and 17.2% after. Past that I had to count the queries that hit zero and read them.
...
To improve it, start with real queries and why they ranked what they did. Then the shape of the distribution. Summary numbers come last. This run is why I put them in that order. Every percentile of Δ came out at 0, and only the queries told me what had broken.
...
Zero-rate and win/tie/loss come out of the same per-query scores if you want them. Win/tie/loss is the one to add when you're comparing two of your own runs, since it needs a baseline scored on the same queries.

What each one buys you, and where it stops:

| what you read          | what it tells you                     | where it stops                      |
| ---------------------- | ------------------------------------- | ----------------------------------- |
| mean                   | one number you can compare            | hides both tails                    |
| percentiles            | the weak tail, in absolute terms      | goes flat when you measure a change |
| win/tie/loss           | how many queries moved, and which way | says nothing about why              |
| the queries themselves | why                                   | doesn't fit on a dashboard          |
