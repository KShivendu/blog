...
- **294 of the 599 queries never move.** Half the set sits in the no-change bar, so +1.9 is an average over mostly zeros.
- The ones that do move are close to balanced, **172 up against 133 down**, which is why the mean lands so near zero.
- Both tails reach the ends anyway. **5 queries lost more than 50 points** and **11 gained more than 50**, on a change that reports as +1.9.

Now let's drill down into the real data and read the queries behind those bars.
