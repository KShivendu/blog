...
Three similar-looking gains, three different changes underneath. One moved a lot of queries in both directions to land there. Another moved far fewer, almost all of them upward. You'd ship those with different confidence, and the mean gives you no way to tell them apart.

Note that there's a drawback compared to reporting percentiles. Win/loss/tie needs a baseline scored on the same queries, so you can't quote it on its own the way you can quote a p25.
