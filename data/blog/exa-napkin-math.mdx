...
Given these, let’s assume Exa’s crawlers curate, sanitize, and create summaries pages (with [LLMs](https://jina.ai/news/reader-lm-small-language-models-for-cleaning-and-converting-html-to-markdown/)) efficiently enough such that:
...
- **Min yearly cost of maintaining Exa 2.0 index with 20B+ docs ~= 90k \* 12 ~= 1.1M \$**
- And I haven't even considered the cost of clustering, crawling, etc. But it could be all done within 1.5-2M$ / year for 20 B docs.
...
- Perplexity [operates at 200B scale](https://research.perplexity.ai/articles/architecting-and-evaluating-an-ai-first-search-api#:~:text=200%20billion%20unique%20URLs) and Exa would probably reach this scale soon with their latest funding, so their cost would shoot up to **15-20M\$ / year minimum just for search part** This makes sense given their latest funding of 85M\$, they need lots of capital for few years of runway.
...
- Query embedding generation with as per [this](https://www.tensoreconomics.com/p/why-are-embeddings-so-cheap#:~:text=despite%20batch%20size%20increasing) on H100 SXM:
  - 1040 tokens with batch size = 1 => 25RPS => 40ms
  - 520 tokens with batch size = 1 => 34RPS => 29ms
  - 260 tokens with batch size = 1 => 37RPS => 27ms
  - 260 tokens with batch size = 8 => 63RPS => 15ms
  - 260 tokens with batch size = 16 => 77RPS => 12ms
  - Assume we have 10 tokens in query (7 words)
    - If batch size = 8 is possible, it should take \< 15ms
    - If batch size = 1, it should take take \< 27ms
  - However, these are numbers for H100s, Exa's query embedding servers might have less powerful GPUs, so let's assume it takes 25ms
  - Remaining latency: 375ms-25ms = 350ms
- Re-ranking queries-docs with cross encoder models: Let's assume another 150-200ms because it's the standard for ranking 100 items against a query. Remaining: 350-200=150ms.
- Vector search + 2 stage re-ranking with uncompressed vectors should be done in around 150ms.
- With Exa 2.0 fast mode, they claim a p50 diff of 79ms and p95 diff of 142ms. This might have come from improvements in:
  - cross encoder re-ranker: They might have been able to afford a distilled or quantized version of the cross encoder re-ranker step because of fine-tuning without severely hurting precision.
  - vector search re-ranking: They might be doing fewer re-ranking steps or with fewer vectors
