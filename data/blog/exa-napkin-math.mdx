...
Update: Exa recently did [improvements](https://exa.ai/blog/exa-api-2-0) across their pipeline to make search faster and more relevant. This means some of my assumptions might be slightly outdated now, but the fundamentals are still applicable. Also, I did some benchmarks before/after their updates so I can compare if they have improved!
...
- Lastly, they do 2nd re-ranking with original 2048d vectors to compensate for all this approximation/compression.
  - Note that model originally generated 4096d vectors. They probably don't use the full 4096 at least in Exa Fast/Auto search
...
- For IVF index with BQ + MRL truncation + re-ranking with original vectors for their "neural search"
  ...
  - Each vector ID needs to be stored in the IVF Index, 20B \* 8 bytes = 160 GB (must be `uint64` to store 4.2B+ docs)
...
- For 1st stage re-ranking with 1024 dim MRL truncated BQ vectors:
  - Storage for 1B docs takes 128GB (1024 / 8 \* 1B / 1e9)
...
- They might be using something like `Qwen3-Reranker-0.6B`
- Public [providers](https://deepinfra.com/Qwen/Qwen3-Reranker-0.6B) charge \$ 0.010 \/ 1M tokens. Let's assume the same rates.
- We have to compare 1 query (10 tokens) against 100 documents (assume avg of 780 tokens). Total tokens: 78k
- Note: This is purely a **search time cost** unlike document embeddings generation which have to be maintained.

| Re-ranker model       | Rate             | Tokens / query | Cost of 1 query |
| --------------------- | ---------------- | -------------- | --------------- |
| `Qwen3-Reranker-0.6B` | $0.0100 / 1M tok | 78k            | 0.00078 \$      |
| `Qwen3-Reranker-4B`   | $0.0250 / 1M tok | 78k            | 0.00190 \$      |
| `Qwen3-Reranker-8B`   | $0.0500 / 1M tok | 78k            | 0.00390 \$      |
...
| Component        | Cost for 1B docs (monthly) | Cost for 20B docs (one-time setup) | 36% overhead for 20B docs (monthly) |
| ---------------- | -------------------------- | ---------------------------------- | ----------------------------------- |
| Content storage  | 0.19k                      | 3.8k                               | 1.37k                               |
| Lexical storage  | 1.89k                      | 37.8k                              | 13.61k                              |
| Vector storage   | 2.75k                      | 55k                                | 19.82k                              |
| Metadata storage | 0.30k                      | 6k                                 | 2.16k                               |
| Embedding cost   | 9.75k                      | 195k                               | 70.2k                               |
| Egress           | 0.09k                      | 1.8k                               | 0.65k                               |
| **Total**        | 17.37k                     | 300k                               | 108k                                |

Notes:

- One-time **setup cost for 20B docs is 300K$**
- **Yearly cost of maintaining Exa 2.0 index ~= 108k \* 12 = 1.3M \$**
...
- Perplexity operates at **200B scale** and Exa would probably reach this scale soon with their latest funding, so their cost would shoot up to **12-18M\$ / year for search part**. This makes sense given their latest funding of 85M\$, they need lots of capital for few years of runway.
...
- Similar flaws of scaling IVF were also demonstrated in [my previous benchmark blog with Nirant](https://nirantk.com/writing/pgvector-vs-qdrant) that compared Qdrant's HNSW and PGVector's IVF Index and our blog led/accelerated the implementation of HNSW in pgvector.
...
- In the query pipeline, vector requires careful planning but takes less time and cost while re-ranking causes more cost and latency.
