...
- RAM Cost: 1200 GB × \$ 2 / GB = $2400 / month
- Disk Cost: 1200 GB × \$ 0.35 / GB = $420 / month
...
After the vector search step, each of the 10 clusters could return 100 items each and then we re-rank them with original vectors. It sounds reasonable. But to read 1k original vectors from disk randomly (i.e. 1k \* 8kb = 8MB of data), it would take 100 μs \* 1k = 100ms which is horrible.

However, assuming a cache hit ratio of 90%, you'll only have to read 100 vectors from disk. Then it takes:

- Disk: 100 μs \* 100 = 10ms
- RAM: 50 ns \* 900 = 47 μs

And assuming a cache hit ratio of 70%, you'll only have to read 300 vectors from disk. Then it takes:

- Disk: 100 μs \* 300 = 30ms
- RAM: 50 ns \* 700 = 35 μs
...
- In this [blog](https://blog.vespa.ai/billion-scale-knn-part-two/), the experiments shows a machine with 72 vCPUs dealing with 1B 100dim vectors that are binary quantized. It shows search latency of HNSW with recall 50% within 2ms and 4ms with 90% recall. It also handles realtime indexing at 15.4k RPS and with 72 cores it means avg write latency is RPS = cores \* 1/latency => latency = cores / RPS = 72 / 32_000 = 4.68ms (just a little higher than search because hnsw mainly needs to search and then it knows where to add new links)
- Also the avg search time complexity of HNSW is `log(N)` while IVF is `log(sqrt(N))`. In theory, it means that to go from 1M to 10B, IVF latency balloons by `math.sqrt(1e10) / math.sqrt(1e6) = 100x` while HNSW search time only increases by `log(1e10) / log(1e6) = 10/6 = 1.6x`. So even if you asssume they perform similar at 1M scale - say 2ms with single thread. HNSW should remain within 5ms even while IVF index search would reach 100-200ms. This is kinda aligned with what we see in numbers reported by Exa benchmarks as 420ms is p50 and p95 is 600ms.
- Something similar was also proven in [my previous benchmark blog](https://nirantk.com/writing/pgvector-vs-qdrant) with Nirant that compared Qdrant HNSW and PGVector IVF Index and it led to implementation of HNSW in pgvector.
- The cost of building and storing index would rise because of higher RAM and CPU requirements for HNSW. Perplexity uses HNSW and hence was able to claim lower latency in their recent benchmarks against Exa. Imo it makes sense for Exa to also do this and so they get more time to spend on re-ranking and LLM based filtering (websets) afterwards.
