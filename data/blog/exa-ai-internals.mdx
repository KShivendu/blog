...
- Total is 500 \$ / month for 1B docs
...
This has downsides because it increases latency a lot. The round trip time from India is `600ms`. They will expand as they get more users across the globe.
...
Exa just raised 85M$\ and valuation must be around , assuming 10x multiple, they might be making 5M\$ in ARR
...
<!-- If you're saturating all 5 every second, you'll reach 5\$ and serve 5 * 5 * 1000 = 25k requests in that time in 1000/5 = 200s. -->

Exa AI python SDK downloads
...
- Embedding generation: 60ms assuming 32 batch size based on [this](https://github.com/huggingface/text-embeddings-inference)
- Vector search + Text + Re-ranking with uncompressed vectors. remaining: 420-60-140=220ms
- Re-ranking with cross encoder models: Takes another 140ms

Let's say search finishes in 20ms. How much disk can you read in 200ms? You can read 8KB pages (= one fp16 4096 dim vector) randomly, you can read 1MB in 15ms. This means 1000 / 8 = 125 vectors in 15ms. So when you have 200, you can read 200/15\*125=1600 vectors randomly. This makes sense, we can read 1000 original vectors from disk. Each of the 10 clusters could return 100 items each and then we rerank them. This sounds sensible.
