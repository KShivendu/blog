...
**No fixed width fixes this, wider just wastes more** (see the glossary above for why cl100k/o200k need 3 bytes at all). A frequency-sorted vocabulary gets there without any entropy coder, reaching 2.89-2.90x. Swapping naive varint for `streamvbyte`/`varintgb` (real SIMD-vectorized libraries from the FastPFOR family) trades a bit of ratio for a ~15-17x faster decode than ANS, a genuine, deployable middle ground.
