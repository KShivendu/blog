...
- **Store text as tokens, not UTF-8 bytes.** Vector databases compress payloads with LZ4 (~1.15× per document). Raw r50k uint16 token IDs alone already reach 2.33×, beating every byte codec including brotli, with no compression algorithm running at all. Token-native storage — BPE token IDs + static ANS — pushes that to 3.37× lossless and aligns the on-disk format with how embedders, rerankers, and LLMs actually consume text.
