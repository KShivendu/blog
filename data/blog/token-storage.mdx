  ...
  data={[
    {
      ...
      x: storageData.x,
      y: storageData.y,
      marker: { color: storageData.color, opacity: storageData.opacity },
            ...
            args: [
              {
                x: [storageData.x],
                y: [storageData.y],
                'marker.color': [storageData.color],
                'marker.opacity': [storageData.opacity],
                text: [storageData.text],
              },
              [0],
            ],
  ...
  data={[
    {
      ...
      name: 'encode (raw)',
      offsetgroup: 'encode',
      x: latencyData.categories,
      y: latencyData.encRaw,
      marker: { color: '#60a5fa', opacity: latencyData.rawOpacity },
      text: latencyData.encRawText,
      textposition: 'inside',
    },
    {
      type: 'bar',
      ...
      offsetgroup: 'decode',
      x: latencyData.categories,
      y: latencyData.decRaw,
      marker: { color: '#10b981', opacity: latencyData.rawOpacity },
      text: latencyData.decRawText,
      textposition: 'inside',
    },
    {
      type: 'bar',
      ...
      offsetgroup: 'encode',
      x: latencyData.categories,
      y: latencyData.encAns,
      marker: { color: '#1d4ed8' },
      text: latencyData.encAnsText,
      textposition: 'inside',
    },
    {
      type: 'bar',
      ...
      offsetgroup: 'decode',
      x: latencyData.categories,
      y: latencyData.decAns,
      marker: { color: '#065f46' },
      text: latencyData.decAnsText,
      textposition: 'inside',
    },
  ]}
  layout={{
        ...
        buttons: [
          {
            ...
            args: [
              {
                x: [
                  latencyData.categories,
                  latencyData.categories,
                  latencyData.categories,
                  latencyData.categories,
                ],
                y: [latencyData.encRaw, latencyData.decRaw, latencyData.encAns, latencyData.decAns],
                'marker.opacity': [
                  latencyData.rawOpacity,
                  latencyData.rawOpacity,
                  latencyData.fullOpacity,
                  latencyData.fullOpacity,
                ],
                text: [
                  latencyData.encRawText,
                  latencyData.decRawText,
                  latencyData.encAnsText,
                  latencyData.decAnsText,
                ],
              },
              [0, 1, 2, 3],
            ],
...
| Tokenizer | uint32 | uint16/3-byte (practical) | + freq-remap, varint | + freq-remap, streamvbyte |                  ANS |
| --------- | -----: | ------------------------: | -------------------: | ------------------------: | -------------------: |
| r50k      |  1.16x |            2.31x (uint16) |                2.95x |          2.64x (~1µs dec) | 3.37x (~15-17µs dec) |
| cl100k    |  1.12x |            1.49x (3-byte) |                2.89x |          2.58x (~1µs dec) | 3.30x (~15-17µs dec) |
| o200k     |  1.13x |            1.50x (3-byte) |                2.90x |          2.60x (~1µs dec) | 3.32x (~15-17µs dec) |

**No fixed width fixes this, wider just wastes more** (see the glossary above for why cl100k/o200k need 3 bytes at all). A frequency-sorted vocabulary gets there without any entropy coder, reaching 2.89-2.90x. Swapping naive varint for [`streamvbyte`](https://github.com/lemire/streamvbyte) (a real SIMD-vectorized library from the FastPFOR family) trades a bit of ratio for a ~15-17x faster decode than ANS, a genuine, deployable middle ground.
...
Note that, as an end user (or DB engine), you don't have to wait for that. You can already start by storing token IDs instead of raw bytes. Even the frequency sorting is just a lookup table, you can apply it in your own storage layer today and achieve more compression than what a vendor's frequency-sorted token IDs would provide (because it's especially assigned for your own corpus)
...
An LLM produces token IDs before the inferene engines detokenize them, so storing generated output (chat history, agent thinking traces) this way has zero encode cost. It splits elegantly by workload: writing-heavy (agent traces, synthetic data) gets free token IDs, reading-heavy (RAG, agent memory) skips retokenize on every future reads. Human reads stay cheap either way, detokenizing is ~4-12µs.
...
- The compression gains are generalizable and across all tokenizers from all vendors. But another massive bonus is never having to tokenize again as models read your data thousands or millions of times in future.
- Although token native storage is already usable and benificial, the ecosystem needs to evolve to make it even better: models don't share a vocabulary yet, and hosted LLM APIs still only take text in and text out.
