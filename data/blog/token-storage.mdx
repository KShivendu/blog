...
Every vector search engine stores two things for each point: the vector and the payload. The community has gone out to compress the vectors with quantization and MRL. However, the text payload gets raw UTF-8, or LZ4 at best, even though the **systems that actually consume it, embedders, rerankers, LLMs, all operate on tokens, not bytes**.

I found that **Token IDs alone, with no compression algorithm running at all, already achieves 2.33× compression over raw UTF-8 bytes**. A static entropy based compression on top reaches 3.35×.

This post argues for **token-native storage**: persist text as BPE token IDs, not UTF-8 bytes. This mismatch of the interface isn't unique to vector search engines, it shows up **anywhere models read and write: web, search, LLM agent traces (or chat history)**.
...
cl100k has a larger vocabulary so it learns longer merges, which usually (but not always) means fewer tokens per document, especially for code and non-English text. Either way, r50k fits in 2 bytes (uint16) while c100k needs 4 bytes (uint32) since the vocab exceeds the uint16 limit of ~65k (=2^16).
...
```
6 bytes/word (1 char = 1 byte) × 3/4 words/token = 4.5 bytes/token (raw text)
...
<details>
<summary>Full results table: min / median / max per method</summary>
...
<PlotlyChart
  ...
  layout={{
    ...
    yaxis: { title: 'microseconds (log scale)', type: 'log' },
    barmode: 'stack',
    height: 420,
...
<details>
<summary>Full latency table: median and p99</summary>
...
<details>
<summary>How big is the text next to the vector, really?</summary>
...
That LZ4 codec (see [the benchmark](#the-benchmark) above) has no learned model of the language, just whatever byte patterns happen to repeat within its window. Token-native storage swaps it for the tokenizer's own output as the on-disk representation, instead of throwing token IDs away right after tokenizing:
