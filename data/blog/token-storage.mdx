...
The same symmetry applies when the writer is a model: a sampler produces token IDs before anything detokenizes them, so storing generated output (chat logs, agent traces) this way has zero encode cost. It splits cleanly by workload: writing-heavy (agent traces, synthetic data) gets free token IDs, reading-heavy (RAG, agent memory) skips detokenize-then-retokenize on every future read. Human reads stay cheap either way, detokenizing alone is ~4µs.
