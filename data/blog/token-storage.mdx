...
3. 3-byte packing alone (no entropy coder) already recovers most of the gap for cl100k/o200k, both of which lose badly under naive 4-byte packing, dropped from the chart above since it's not a real recommendation (see the [glossary](#glossary) for why). **ANS recovers further still, to 3.30x (cl100k) and 3.32x (o200k)**.
