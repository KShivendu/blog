...
Two separate claims live in this post, and it's worth keeping them apart: **token-native storage itself** (store IDs, not bytes — the core argument) is the settled part. **Which entropy coder to layer on top** (ANS, frequency-remap, something else) is a genuinely open sub-problem I'm still actively working — see the [faster-alternative section](#a-faster-alternative-to-ans-frequency-sorted-token-ids) and its own follow-up experiments. Don't read "ANS gets 3.37x" as the ceiling of the idea; read "storing IDs instead of bytes" as the idea, with the coder as a swappable, improvable detail.
...
WikiText is one curated corpus, though — I reran this full comparison on real, non-overlapping prose/code/Hindi corpora at realistic RAG chunk sizes (256-2,000 tokens) in a [follow-up post](/blog/token-storage-extra#does-335-hold-outside-wikitext-103), toggle-able by dataset. The headline number holds up reasonably well on prose (~3.3-3.4x) and code (~3.1-3.2x), but Hindi is the interesting outlier: r50k and cl100k raw-pack it to **0.84-0.87x, worse than doing nothing**, because neither tokenizer ever learned real Devanagari merges, while o200k (which did) reaches **5.5-6.1x** on the exact same text. Same idea, same method, opposite outcome — purely a function of whether your tokenizer's vocabulary actually covers your script.
...
3. ANS decode looks slow next to LZ4's raw decompress — but that comparison isn't fair to the actual consumer. An agent needs token IDs regardless of storage format, so a byte-codec payload still owes it a tokenize step _after_ decompressing, a cost the naive comparison doesn't charge. Charge it fairly and ANS decode comes out **2-8x faster** than decompress+tokenize, measured across prose/code/Hindi in the [follow-up post](/blog/token-storage-extra#does-the-faster-than-ans-alternative-hold-outside-wikitext-too). ANS is still the slower of the two token-native options below it, though, which is why the frequency-sorted alternative still has a place — just not because ANS decode is slow in isolation.
...
This holds up beyond WikiText too: re-measured on prose/code/Hindi at 256-2,000 token chunks (same [follow-up post](/blog/token-storage-extra#does-the-faster-than-ans-alternative-hold-outside-wikitext-too)), frequency-remap consistently decodes 1.5-5.7x faster than ANS for roughly 15-26% less compression, at every domain and chunk size tested. If your workload weighs decode latency heavily (an agent-serving hot path), it's a competitive default, not just a fallback. If ratio matters most (cold storage, egress, backups), ANS keeps a real, consistent edge. Nothing stops mixing the two — ANS at rest, frequency-remap for a hot cache tier — since both round-trip through the same token IDs.
