...
3. cl100k and o200k just need the right container: switching from naive uint32 (4-byte) packing (left off the chart above, see the [glossary](#glossary) for why) to 3-byte packing, with no entropy coder at all, already gets them to 1.49x and 1.50x. **ANS recovers further still, to 3.30x (cl100k) and 3.32x (o200k)**.
