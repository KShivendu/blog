...
Decode splits into two very different numbers depending on who's reading: an agent wants token IDs and stops there, a human wants text, which means detokenizing on top. **Human** decode = full round trip to text (unpack/ANS-decode + detokenize for token-native methods; byte codecs are already text, so just decompress). **Agent** decode = only what it takes to get token IDs: raw/+freq/+ANS skip detokenize entirely, while byte codecs (which only decompress to bytes) still owe a tokenize step, charged here. Encode is identical either way, since the write path doesn't know who'll read it later.

// +freq decode = streamvbyte-decode + rank-lookup (+ detokenize for Human).
// Agent-mode ANS/raw/freq skip detokenize; byte codecs pay decompress + tokenize
// instead, since an agent needs IDs and a byte-codec payload doesn't have them.
// decAgent for raw/ANS/byte-decompress is the median of 30 back-to-back reps
// per chunk, not a single perf_counter() shot: a single shot on a sub-10us op
// (e.g. r50k's raw unpack, a zero-copy np.frombuffer reinterpret) is dominated
// by GC/scheduler noise, not the operation, and the first cut of this chart
// briefly showed r50k unpack SLOWER than LZ4 decompress because of exactly that.
export const latValues = {
english: {
enc: [11.4, 107.1, 704.2, 8226.9, 1212.1, 439.8, 508.0, 315.7, 553.5, 328.3, 383.2, 505.5, 563.9, 412.0],
decHuman: [4.4, 34.5, 25.9, 63.3, 18.5, 81.1, 86.7, 87.2, 170.1, 81.8, 174.6, 199.0, 207.0, 235.7],
decAgent: [449.7, 459.1, 455.3, 467.2, 454.1, 1.3, 13.8, 13.5, 30.3, 13.8, 29.6, 36.8, 38.6, 41.7],
},
code: {
enc: [5.8, 55.8, 391.1, 4580.0, 731.9, 326.0, 177.6, 210.4, 443.5, 372.0, 154.2, 397.7, 380.1, 271.6],
decHuman: [3.0, 19.7, 12.8, 40.4, 12.7, 48.3, 22.8, 45.1, 138.3, 97.7, 57.1, 150.7, 126.5, 142.3],
decAgent: [347.8, 358.1, 350.6, 356.1, 348.1, 1.4, 12.5, 12.0, 26.5, 24.5, 10.1, 38.0, 23.9, 25.1],
},
hindi: {
enc: [5.2, 60.6, 383.3, 3414.7, 614.0, 248.0, 213.5, 115.3, 337.1, 174.9, 83.3, 305.8, 292.7, 157.1],
decHuman: [2.6, 18.7, 18.5, 40.3, 5.3, 31.5, 29.5, 27.6, 103.1, 51.7, 32.7, 127.8, 110.0, 88.3],
decAgent: [103.6, 112.7, 107.8, 111.4, 102.2, 1.3, 12.6, 11.5, 21.2, 11.0, 7.6, 34.8, 25.6, 13.8],
},
}
...
export const latVariant = (domain, mode, idx) => {
// Same per-method colors as the ratio chart, so a method is identifiable
// by color in both charts. encode/decode are told apart by opacity instead
// of hue: encode dimmed, decode at the method's own ratio-chart opacity.
...
const decOps = idx ? idx.map((i) => ratioOpacities[i + 1]) : ratioOpacities.slice(1)
const encOps = cols.map(() => 0.45)
const encSrc = latValues[domain].enc
const decSrc = mode === 'agent' ? latValues[domain].decAgent : latValues[domain].decHuman
const enc = idx ? idx.map((i) => encSrc[i]) : encSrc
const dec = idx ? idx.map((i) => decSrc[i]) : decSrc
return {
categories: cats,
series: [
{ name: 'encode', group: 'encode', colors: cols, opacities: encOps, values: enc, text: fmtUs(enc) },
{ name: 'decode', group: 'decode', colors: cols, opacities: decOps, values: dec, text: fmtUs(dec) },
],
}
}
export const latScales = [
{
label: 'Log',
valueScale: 'log',
valueMin: 1,
valueMax: 10000,
valueTicks: [1, 10, 100, 1000, 10000],
valueLabel: 'microseconds (log scale)',
},
{ label: 'Linear', valueScale: 'linear', valueLabel: 'microseconds' },
]
export const latDataset = (domain, mode) => ({
label: domain[0].toUpperCase() + domain.slice(1),
...latVariant(domain, mode),
variants: [
{ label: 'All', ...latVariant(domain, mode), scales: latScales },
{ label: 'Focus', ...latVariant(domain, mode, latFocusIdx), scales: latScales },
],
})
...
<BarChart
  ...
  stacked
  title="Encode/decode latency, median, 512-token chunks"
  subtitle="toggle dataset (left), Agent/Human decode (right), All/Focus and Linear/Log (below)"
  valueUnit="µs"
  views={[
    { label: 'Agent', datasets: ['english', 'code', 'hindi'].map((d) => latDataset(d, 'agent')) },
    { label: 'Human', datasets: ['english', 'code', 'hindi'].map((d) => latDataset(d, 'human')) },
  ]}
/>

Observations:

1. **The two modes tell opposite stories about which method is fast, that's the point of splitting them.** LZ4 "decodes" in 2.8-4µs, but only if nobody needs token IDs. Switch to Agent (the default) and LZ4 jumps to 100-470µs, because now it owes a tokenize step, same as every byte codec. Meanwhile raw/+freq/+ANS drop to single-digit-to-40µs in Agent mode (they already have IDs, no detokenize needed) and only get expensive in Human mode.

2. **Tokenization is the cost that never goes away, only who pays it and when changes.** Byte codecs defer it to every single read. Token-native methods pay it once at write time and never again, which is why their Agent-mode decode numbers are so much smaller: they're not skipping the work, they already did it.

3. In Agent mode, ANS decode comes out **2-8x faster** than a byte codec's decompress+tokenize ([full breakdown](/blog/token-storage-extra#does-the-faster-than-ans-alternative-hold-outside-wikitext-too) across English/code/Hindi). It's still slower than `+freq` though, which is the trade the [next section](#a-faster-alternative-to-ans-frequency-sorted-token-ids) is about.

4. brotli's encode cost is brutal regardless of mode, 3.4-8.2ms per chunk, 10-20x slower than zstd for a ratio gain that ANS beats anyway without the wait.
