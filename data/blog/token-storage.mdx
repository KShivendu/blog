...
- **The actual argument: store text as tokens, not UTF-8 bytes.** Raw r50k uint16 token IDs alone already reach 2.33×, beating every byte codec, with zero entropy coding at all. Any coder layered on top (ANS, frequency-remap) is a bonus, not the point.
- **Token-native means reading and writing in whatever representation the model already speaks.** LLM output (chat logs, agent traces) is born as token IDs, so storing it that way has zero encode cost, no detokenize-then-recompress round trip.
- Optional entropy coding pushes the ratio further, at a real latency cost: a shared static ANS table reaches 3.37× lossless (encode ~50-65µs, decode ~19-21µs), or frequency-sorted IDs + streamvbyte gets ~78-87% of that ratio at ~1/15th the decode latency, a change any tokenizer vendor could ship for free.
- The compression multiplier, whichever coder you pick, applies to **every copy of the bytes** — snapshots, WAL, replication, shard rebalancing, page-cache density, egress — not just primary storage.
- **Not an OpenAI-only story**: cl100k, o200k, Qwen2.5, DeepSeek-V2, and Gemma-2 all show the same packing pattern on their own real tokenizers.
- Real limitations to respect: no shared vocabulary across models yet, hosted LLM APIs don't accept token IDs today, and the vocabulary becomes part of your on-disk format forever, so version your encodings.
