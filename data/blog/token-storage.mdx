...
- Just switching from UTF-8 bytes to token IDs gets you free compression, no entropy coder needed. r50k alone hits 2.33x, already beating every byte codec. ANS on top gets you to 3.37x, but that's a bonus, the tokens are the real win.
- This isn't an OpenAI trick. cl100k, o200k, Qwen2.5, DeepSeek-V2, Gemma-2, all of them show the same pattern on their own real tokenizers.
- The hard part isn't the math, it's the ecosystem: models don't share a vocabulary yet, and hosted LLM APIs still only take text in and text out.
