...
The insert call barely changes. `description` still takes the same value as before, plain text, and the database tokenizes it on write. A client that already has token IDs on hand can send those instead, as a plain array, no wrapper object needed:
  ...
  "points": [
    {
      "id": 123,
      "vector": [0.12, -0.34, ...],
      "payload": { "description": [1858, 6427, 20272, 318, 257, 6997, ...] }
    }
  ]
}
```
