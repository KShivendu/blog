...
subtitle: "Read and Write in Your Agent's Language"
date: '2026-05-12'
lastmod: '2026-08-04'
...
export const ratioFocusIdx = [0, 1, 3, 6, 8, 9, 11, 12, 14] // Raw UTF-8, LZ4, zstd-19, r50k raw, o200k raw, r50k +freq, o200k +freq, r50k +ANS, o200k +ANS
...
export const latFocusIdx = [0, 2, 5, 7, 8, 10, 11, 13] // LZ4, zstd-19, r50k raw, o200k raw, r50k +freq, o200k +freq, r50k +ANS, o200k +ANS
...
- **+freq**: token IDs re-ranked by real-world frequency, then packed with [`streamvbyte`](https://github.com/lemire/streamvbyte), a variable-length integer codec. No entropy coder either, just a smarter container (see [how it works](#step-2-extra-token-id-compression) above).
...
Unquantized, the embedding (4,096 B) dominates the text either way, most collections with big uncompressed embeddings look like this. MRL brings it closer: 512 or 256 dims lands at 2,048 B or 1,024 B, same ballpark as the text instead of dwarfing it. [int8 quantization](https://qdrant.tech/articles/scalar-quantization/) lands in the same place, 1,024 B. [Binary quantization](https://qdrant.tech/articles/binary-quantization/) is the real outlier: 128 B, ~5.5x smaller than the compressed text chunk (707 B), though check your recall numbers before leaning on it that hard.
...
Default stays `"text"`, so existing clients see no change. An LLM-facing pipeline asks for `"tokens"` and skips detokenization; anything human-facing asks for `"text"`. This only works end to end if you own the inference stack, since hosted LLM APIs take text prompts, not token arrays ([yet](/blog/token-api)).
