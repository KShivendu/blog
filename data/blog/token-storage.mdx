...
This splits cleanly by workload. Writing-heavy, where LLMs generate most of what gets stored (agent traces, synthetic data, summaries), you get token IDs for free, there's no encode step to pay for. Reading-heavy, where LLMs are the ones pulling documents back out (RAG, agent memory), every future read skips detokenize-then-retokenize entirely, so the cost that matters most just disappears. Human reads stay cheap regardless: detokenizing is the fast half of the round trip, ~4µs in this post's numbers, so a page rendering to a person barely notices the extra step.
...
- The compression gains are generalizable and across all tokenizers from all vendors. But another massive bonus is never having to tokenize again as models use your data thousands or millions of times in future.
- Although token native storage is already usable, the ecosystem needs to evolve to make it even better: models don't share a vocabulary yet, and hosted LLM APIs still only take text in and text out.
