  ...
  data={[
    {
      type: 'bar',
      orientation: 'h',
      x: [1.0, 1.15, 1.66, 1.68, 2.26, 2.33, 1.13, 1.49, 2.61, 3.37, 3.3],
      y: [
        'Raw UTF-8',
      ...
      marker: {
        ...
        opacity: [0.75, 0.75, 0.75, 0.75, 0.75, 1, 1, 1, 0.75, 1, 1],
      },
      text: [
        '1.0×',
        '1.15×',
...
3. cl100k raw uint32 sits near 1.13x, worse than even gzip, but most of that is the container, not the tokenizer: just switching to a 3-byte packing (cl100k's 100,277 tokens still fit in 24 bits) already recovers 1.49x, no entropy coder involved. **cl100k + ANS recovers further to 3.30x**: the fixed-width storage penalty vanishes once ANS allocates bits by frequency instead of by vocab size.
...
## A Cheaper Fix: Frequency-Sorted Token IDs

ANS gets close to the entropy bound, but it isn't the only lever, and it isn't the cheapest one. **BPE assigns token IDs in merge-discovery order during training, not by how often a token actually appears in real text.** I checked this directly: the Spearman correlation between a token's default ID and its real-world frequency rank is only 0.50. Of the 1,000 most frequent tokens, only 33% land in the first 1,000 IDs.

That gap is wasted, because fixed-width packing (uint16, uint32) can't use it at all. Every token costs the same number of bytes regardless of its ID, 2 bytes is 2 bytes whether the ID is 5 or 50,000. Turning "small ID" into "fewer bytes" needs a variable-length integer code on top, but tried alone on BPE's default ID order, that doesn't help either: plain varint on the default order actually came out worse than uint16 packing (2.23x vs 2.29x for r50k), since the IDs aren't sorted enough for "smaller number, fewer bytes" to pay off.

**Combine both and it works.** Reassign token IDs by frequency rank, then use a variable-length integer code:

| Tokenizer | uint16/32 (current) | Freq-remap + varint | ANS (existing benchmark) |
| --------- | ------------------: | ------------------: | -----------------------: |
| r50k      |               2.29x |               2.93x |                    3.37x |
| cl100k    |               1.09x |               2.85x |                    3.30x |

cl100k is the real story. Its raw packing is currently worse than gzip purely because 100,277 tokens don't fit in uint16, forcing 4 bytes per token. A frequency-sorted vocabulary plus a variable-length code fixes that outright, 1.09x to 2.85x, with no entropy coder, no probability model, and no per-corpus training beyond the one-time frequency count you'd want anyway.

The variable-length step doesn't have to be slow, either. Swapping the naive per-integer varint for `streamvbyte` or `varintgb` (real, SIMD-vectorized libraries from the FastPFOR family, used in production search engines) gets:

| Tokenizer | Freq-remap + streamvbyte | Decode latency |
| --------- | -----------------------: | -------------: |
| r50k      |                    2.62x |           ~1µs |
| cl100k    |                    2.53x |           ~1µs |

That's roughly 15-17x faster to decode than ANS (15-17µs), for about 78% of ANS's compression ratio. A real, honest tradeoff, not a free lunch, but a genuinely deployable one.

**This is the actual ask of this post.** If a tokenizer vendor assigned token IDs by frequency rank at training time instead of merge-discovery order, this fix would cost nothing extra: it's just a choice of which integer to assign to which token, made once, at vocabulary-construction time. It changes neither the tokenizer's behavior nor the model, only which number represents each token. For a model already shipped, the same fix is still cheap: permute the embedding table rows to match the new ID order, a one-time mechanical operation, not a retrain, and every downstream system that stores token IDs benefits immediately.

You don't have to wait for that. The remap is just a lookup table built from a frequency count, the same one this post already needs for ANS, so you can apply it entirely inside your own storage layer today: translate between the model's ID and a frequency-sorted storage ID at the tokenize/detokenize boundary, no changes to the tokenizer or the model required.
