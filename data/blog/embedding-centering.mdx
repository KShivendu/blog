...
draft: true
summary: 'Subtracting a single precomputed mean vector from mxbai/bge/gte embeddings improves discrimination by 65% with no model changes, no retraining, and negligible compute. This post explains why the cone exists, why centering dismantles it, why full whitening backfires, and how a vector database could implement this automatically.'
images: []
authors: ['default']
---
...
Here is the experiment result first:
...
Subtracting a single 1024-dim vector — the corpus mean — eliminates anisotropy completely, improves hard-negative discrimination by 65%, and leaves STS-B correlation essentially unchanged. Full whitening actually hurts. The simple thing wins.
...
The SNLI discrimination gap measures how well the model separates contradiction pairs from random cross-pairs. Raw mxbai: +0.212. Centered mxbai: +0.350. That's a 65% improvement from a one-time subtraction.

To put this in retrieval terms: the model's ability to push hard negatives below true matches improves by 65% without any change to the model, training data, or index structure. For production systems where hard negatives are the failure mode (and they always are — easy cases work without dense retrieval), this is the highest-ROI post-processing step available.

The centering vector is portable: compute it once on a background corpus, reuse it across all collections using the same embedding model. If Mixedbread published a precomputed mean for mxbai-embed-large-v1 (a 1024-float file, 4KB), every user would benefit immediately.

The cone exists because MLM pretraining didn't optimise for isotropy. Centering is the minimal surgical fix that removes exactly the non-informative component the pretraining introduced, leaving the 55-dimensional semantic signal intact and uncompressed.

It is, genuinely, a free lunch.
