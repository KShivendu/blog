...
def estimate_unique_values(r):
    with r.pipeline() as pipe:
        pipe.scard('unique_values')
        return pipe.execute()[0]

def estimate_unique_values_hll(r):
    with r.pipeline() as pipe:
        pipe.pfcount('unique_values_hll')
        return pipe.execute()[0]
...
def main():
    ...
    print(f"Time taken to insert values without HyperLogLog: {end_time - start_time:0.2f} seconds")

    start_time = time.perf_counter()
    unique_value_count = estimate_unique_values(r)
    end_time = time.perf_counter()
    print(f"Time taken to estimate unique values without HyperLogLog: {end_time - start_time} seconds")
    ...
    end_time = time.perf_counter()
    print(f"Time taken to insert values with HyperLogLog: {end_time - start_time:0.2f} seconds")

    start_time = time.perf_counter()
    unique_value_count_hll = estimate_unique_values_hll(r)
    end_time = time.perf_counter()
    print(f"Time taken to estimate unique values with HyperLogLog: {end_time - start_time} seconds")
...
Redis also supports Bloom filters, which are probabilistic data structures that can be used to test whether an element is a member of a set. Bloom filters are memory-efficient and can be used to test whether an element is a member of a set that is too large to fit in memory. Some real world use cases for Bloom filters include: checking whether a URL has been visited/crawled before, checking whether an username/email has been registered before, checking whether a credit card number is valid, checking whether a transaction has been processed before.

Here is an example of using Bloom filters in Redis:

[//]: <> (TODO: Add example of using Bloom filters in Redis)

```bash
127.0.0.1:6379> BF.ADD my_bloom_filter 1
(integer) 1
127.0.0.1:6379> BF.EXISTS my_bloom_filter 1
(integer) 1
```

In this example, we used the BF.ADD command to add the value 1 to the Bloom filter, and the BF.EXISTS command to test whether the value 1 is a member of the Bloom filter. The BF.EXISTS command returns 1 because the value 1 is a member of the Bloom filter. If we had used the BF.EXISTS command to test whether the value 2 is a member of the Bloom filter, it would have returned 0 because the value 2 is not a member of the Bloom filter. The BF.EXISTS command returns 0 with a probability of false positives, which is configurable when the Bloom filter is created.
