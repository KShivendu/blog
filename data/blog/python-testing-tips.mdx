...
I encountered a situation where test A was failing when running with other tests but not when running in isolation. So I wanted to do a binary search against the list of tests to find the culprit. I wrote a small script to do that. I think it's worth sharing:
...
This was super helpful in finding the culprit which helped in catching a disastrous but small bug in the code.

I hope this will help someone get an idea of how tests can improve your code's robustness. Feel free to reach out to me on [Twitter](https://twitter.com/_KShivendu) or [LinkedIn](https://www.linkedin.com/in/kshivendu/) (Please add a message while sending the connection request)
