...
Note: Differences between primary key and UNIQUE constraints:
...
- Hwoever `UNIQUE` does not imply that they are the main identifier of the table. A table can have multiple unique constraints. A column with a unique constraint can contain null values, but only one null value is allowed.
...
## Getting an approx sum in postgres:

- https://www.youtube.com/watch?v=eI_EQNTxF6U => This is one apporoach but many people commented that it's not good in practical cases
- Some mentioned that they just maintain a count seperately and keep increasing/decreasing it..
- Someone mentioned that "Visibility Map (VM) clean with frequent vacuum and then use index-only counts, which are fast"
-
