...
This is the opposite of what you might expect. The intuition "more dimensions = better discrimination" doesn't hold within the Matryoshka structure. Here's why: MRL trains the model such that the first 64 dimensions already form a complete, independent embedding that minimizes the contrastive loss. The next 64 (dims 64–128) add orthogonal information that further reduces loss. By the time you reach dims 512–1024, those dimensions are capturing the finest-grained refinements — but they're also the dimensions that contribute most to making similar sentences look _more_ similar (the absolute magnitudes are smaller, so they drag the full-vector cosine toward the slice-average).

Put differently: the early Matryoshka tiers encode the coarsest but most contrastive signal. The later tiers encode fine-grained nuance. When you measure on contradictions — where you _want_ low similarity — the early tiers win.
