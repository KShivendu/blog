...
Measured across 1,773 [WikiText-103](https://huggingface.co/datasets/Salesforce/wikitext) test articles (≥30 words). The ANS frequency table was trained on the WikiText-103 train split — a shared static model, no per-document overhead.

| Method                     |       min |    median |       max |
| -------------------------- | --------: | --------: | --------: |
| LZ4 (default in Qdrant/ES) |     ~1.0x |     ~1.3x |     ~1.5x |
| gzip -9                    |     0.99x |     1.66x |     2.34x |
| zstd -22                   |     1.11x |     1.68x |     2.39x |
| brotli q=11                |     1.22x |     2.26x |     3.13x |
| r50k BPE uint16 (raw)      |     1.04x |     2.33x |     3.34x |
| cl100k BPE uint16 (raw)    |     0.89x |     2.27x |     3.24x |
...
- **r50k BPE uint16 (raw)**: OpenAI's r50k tokenizer (vocab 50k) IDs stored as raw 2-byte integers — no entropy coding at all, just the ID array.
- **cl100k BPE uint16 (raw)**: OpenAI's cl100k tokenizer (vocab 100k, used by GPT-4/embeddings) IDs stored raw. Larger vocabulary = fewer tokens per doc but larger IDs, so raw storage is worse than r50k. Can expand very short docs (min < 1x).
...
- **r50k / cl100k BPE + static ANS**: tokenize, then entropy-code the token IDs with ANS using a static frequency table trained on WikiText-103. The frequency table is shared — zero per-document overhead. r50k wins slightly because its smaller vocabulary has higher per-token information density.

</details>

Two things stand out. Raw uint16 token IDs — no compression algorithm at all — already beat every byte codec except brotli. And r50k + static ANS beats everything by a wide margin, including at p99.
  ...
  data={[
    {
      type: 'bar',
      orientation: 'h',
      x: [1.0, 1.3, 1.66, 1.68, 2.26, 2.33, 2.61, 3.37],
      y: [
        'Raw UTF-8',
      ...
      text: ['1.0×', '1.3×', '1.66×', '1.68×', '2.26×', '2.33×', '2.61×', '3.37×'],
...
LZ4 decompresses in 1µs — it's designed entirely for speed. zstd dict has the slowest encoder (129µs) — we tested 32KB, 64KB, and 112KB dictionaries trained on WikiText-103 and picked 112KB as the best-performing size — but decompresses in 2µs. The tokenizer methods sit comfortably in the middle: ~54µs encode (dominated by the tokenizer, not ANS), ~18µs decode.
