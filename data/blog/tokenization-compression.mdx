...
LZ4 decompresses in 1µs — it's designed entirely for speed. zstd dict has the slowest encoder (129µs) because it searches a 112KB dictionary per compress, but decompresses in 2µs. The tokenizer methods sit comfortably in the middle: ~54µs encode (dominated by the tokenizer, not ANS), ~18µs decode.
