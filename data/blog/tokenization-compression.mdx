...
| Method | encode median | encode p99 | decode median | decode p99 |
| ------ | ------------: | ---------: | ------------: | ---------: |
| r50k   |          53µs |      123µs |          19µs |       45µs |
| cl100k |          64µs |      166µs |          21µs |       52µs |

The tokenizer dominates the encode side. ANS itself adds ~15µs. If your ingest pipeline already tokenizes (for the embedding model, for token counting), compression is nearly free. Decoding is under 25µs, noise next to a disk read or a network hop.
