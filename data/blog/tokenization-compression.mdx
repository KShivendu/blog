...
The catch: this only works cleanly if the tokenizer's vocabulary fits in uint16 (max 65,535). GPT-4's `cl100k` tokenizer has 100,277 tokens — that needs uint32 (4 bytes), which kills the advantage for **raw packing**. GPT-2's `r50k` tokenizer has 50,257 tokens — fits cleanly in uint16. With ANS the story is different, as the table below shows.
