...
draft: false
summary: 'Vector databases compress text payloads with LZ4 by default. Tokenize the text first and entropy-code the token IDs, and you get ~3x lossless compression, 2.6x what LZ4 manages, using pieces every ML stack already ships.'
...
The probability model only controls how many bits each token gets — it doesn't change what's stored. Token IDs are always encoded and recovered exactly. A better model just allocates bits more precisely: `" model"` after `" language"` is far more likely than `" model"` in general, so a bigram table assigns it fewer bits. The decoder already knows the previous token (it just decoded it), so it uses the same table and recovers the exact token ID. Nothing is approximated — think of it as a smarter ruler for measuring bit lengths, not a lossy transform.
...
- Vector databases apply LZ4 to text payloads (1.3x). Tokenizing first and entropy-coding the token IDs gives 3.35x on average. Lossless, 2.6x better than LZ4.
