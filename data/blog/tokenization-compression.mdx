...
- **r50k BPE uint16 (raw)**: OpenAI's r50k tokenizer IDs stored as raw 2-byte integers — no entropy coding at all, just the ID array.
...
- **r50k BPE + static ANS**: tokenize with r50k, then entropy-code the token IDs with ANS using a static frequency table trained on WikiText-103. The frequency table is shared — zero per-document overhead.
...
~5% better compression at the median. The problem is **character error rate (CER)**: 80.4% on average, every single article affected. The culprit is BERT's lowercasing — `"Qdrant"` becomes `"qdrant"`, `"Wikipedia"` becomes `"wikipedia"`. Case is permanently lost; there's no recovering it from the compressed form. Punctuation spacing (`"(content)"` → `"( content )"`) is a separate quirk, fixable with regex:
...
But case loss makes mxbai unsuitable as a general-purpose storage codec. r50k is lossless on all text. The one real risk with WordPiece is its `[UNK]` token, but on standard English including code and URLs, it only fires on emoji and rare Unicode symbols.
