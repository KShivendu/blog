...
This post makes one claim: **tokenize that text before compressing it, and you roughly double what generic byte codecs achieve**. I'll show the benchmark, the latency cost (small), and what it would take to add this to a real database. There's another reason to care beyond the ratio, too: agents, not humans, are increasingly the ones reading this data, and agents read and write in tokens. A **token-native storage format** speaks their language directly and gets the compression win for free.
...
- **Decode is slower than LZ4**, the codec this post is actually trying to replace (see [latency](#what-does-it-cost-in-latency)), chosen precisely because it's fast, not because it compresses well. Both are still tiny next to typical network or disk I/O, so it's a non-issue in absolute terms, but LZ4 wins on raw speed by a wide margin, exactly the tradeoff it was built for.
