...
draft: false
summary: 'Vector databases store raw UTF-8 text payloads with zero compression. BPE tokenization + entropy coding gives you 5x lossless compression using infrastructure already in every ML stack.'
...
| Method     |    1B docs | Monthly S3 cost |
| ---------- | ---------: | --------------: |
| Raw UTF-8  |     4.5 TB |           ~$104 |
| r50k + ANS |    0.88 TB |            ~$20 |
| **Saving** | **3.6 TB** |  **~$84/month** |

S3 cost savings alone are modest at 1B docs. The real win is **I/O throughput**: 0.88 TB loads 5x faster off disk and over network than 4.5 TB. For bulk indexing, backup/restore, and cross-region replication, that matters a lot more than the storage line item.
