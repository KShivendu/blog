---
title: 'Token-Native Storage: Store Text the Way Models Read It'
date: '2026-05-12'
lastmod: '2026-07-08'
...
Every vector search engine stores two things for each point: the vector and the payload. Vectors get amazing compression with quantization and MRL. The text in the payload gets raw UTF-8 bytes, or LZ4 compression at best, a generic codec with no idea the systems that actually consume it (embedders, rerankers, LLMs/agents) all operate on **tokens**. This problem can be found anywhere models/agents read and write: agent traces, LLM chat history, not just vector search.

This post argues for **token-native storage**: persist text as BPE token IDs, not as UTF-8. Token IDs alone, with no compression algorithm running at all, already pack ~2.33× smaller than raw UTF-8 text. Combine that with a static entropy coder and it reaches ~3.35× (2.9× what LZ4 manages). The same bytes shrink everywhere they get copied — snapshots, replication, etc and LLM output can be persisted without detokenizing first. If agents are going to be the biggest users of text based systems like web, search, and chat interfaces; the storage should be designed to speak their language i.e. tokens

This post also argues for a need for new common standard for tokens like ASCII/UTF-8 to amplify the impact of such design across embeddings, rerankers, and LLMs/agents.

## Rethinking the Payload
...
```json
{
  ...
  "vector": [0.12, -0.34, ...],
  "payload": {
    "description": "Token-native storage persists text as token IDs, not UTF-8 bytes..."
  }
}
```
...
But there is a component that already knows a lot about the structure of natural language, and it's part of every embedding and language model stack: **the tokenizer**. Token-native storage means making token IDs the on-disk representation instead of treating tokenization as a transient inference step:

```json
// Collection schema, set once: every document in this collection shares
// one tokenizer and one static ANS table, so they stay decodable together.
{
  "payload_schema": {
    "description": { "type": "token", "tokenizer": "r50k" }
  }
}
```

```json
// Per-document insert: no tokenizer field, it's inherited from the schema.
{
  "id": 123,
  "vector": [0.12, -0.34, ...],
  "payload": {
    "description": {
      "tokens": [1858, 6427, 20272, 318, 257, 6997, ...]
      // or "text": "Token-native storage persists text as token IDs, not UTF-8 bytes...",
      // letting the database tokenize on insert for clients that don't tokenize themselves
    }
  }
}
```
...
Nothing exotic. Token-native storage can be a payload encoding extension:

1. Accept a token-native encoding (e.g. `tokenizer: "r50k"`) as a collection-level schema option
2. Tokenize on insert, pack IDs as uint16, run ANS with the shared static table
3. Decompress transparently on read (or skip converting to UTF-8 if the consumer is an LLM)

There's a bigger-picture reason to like this as [search and generation converge](https://www.kshivendu.dev/blog/why-search#the-duality-of-search:~:text=Unified%20search%20and%20generation): keeping UTF-8 blobs in the index while the model reasons in tokens means maintaining two representations of the same text. Storing token IDs in LLM format (r50k/cl100k) also avoids re-tokenizing retrieved documents at inference time when feeding them to the LLM — the token IDs are already there.

The inference-side wins go further: skipping re-tokenization at serving time, and KV-cache reuse for hot documents. That thread has its own experiments and its own surprises (the hot-doc opportunity turns out to hinge entirely on how concentrated your query traffic is): [The Hot Doc Problem: KV Cache Tiering for RAG](/blog/rag-kv-cache-tiering).
