...
There's another reason to care than the compression ratio: agents, not humans, are increasingly the ones reading unstructured data, and agents think in tokens. A **token-native storage format** for text speaks their language directly and gets the compression win for free.
...
With this napkin math, we see that just packing token IDs into uint16 already compresses, before any actual compression algorithm runs.
...
The downstream effect is bigger than it looks:

- **Token pricing**: unaffected. Billing is per-token at inference time, unrelated to how those token IDs get entropy-coded at rest downstream.
- **Latency**: unaffected at inference. What it removes is the bootstrapping tax every downstream team pays today: this post had to approximate GPT's real distribution using WikiText, an English-only proxy. A real corpus-derived table needs no approximation.
- **Context window**: unaffected. That's a function of vocabulary design (size and merge coverage), a separate lever from frequency-table publishing.
- **Multilingual and code coverage**: this is where a real table wins clearly. GPT's actual training mix already includes code and dozens of languages, so a vendor-published table would generalize where a WikiText proxy can't. Our JSON benchmark showed exactly this failure mode: a text-trained table gave 1.10× on JSON, a JSON-trained table gave 2.24×. A vendor's real table, trained on a mix that already includes structured and multilingual text, would likely land close to the JSON-trained number out of the box, for every content type at once.
