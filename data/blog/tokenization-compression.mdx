...
- **r50k BPE uint16 (raw)**: r50k has 50,257 tokens — fits in uint16 (max 65,535), so 2 bytes per token ID.
- **cl100k BPE uint32 (raw)**: cl100k has 100,277 tokens — exceeds uint16, requires uint32 (4 bytes per token). It produces fewer tokens per document than r50k (larger vocab = longer merges), but 4 bytes × fewer tokens still comes out worse than 2 bytes × more tokens. Hence 1.13x vs 2.33x.
...
- **r50k / cl100k BPE + static ANS**: ANS allocates bits based on token frequency, not a fixed byte width. A common token gets ~1 bit; a rare one gets ~16 bits — regardless of vocab size. So the uint32 penalty that crushes cl100k raw disappears entirely: both tokenizers land near 3.3x.
