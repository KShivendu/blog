...
// Second grey, for the common case of two reference lines rather than one: a plain
// baseline and a tuned baseline, say. Same tint, one lightness step away.
const MUTED_ALT_LIGHT = '#535753'
const MUTED_ALT_DARK = '#a5aba8'

export function vizPalette(dark) {
  const ramp = dark ? RAMP_DARK : RAMP_LIGHT
  const muted = dark ? MUTED_DARK : MUTED_LIGHT
  const series = dark ? SERIES_DARK : SERIES_LIGHT
  const seriesAlt = dark ? SERIES_ALT_DARK : SERIES_ALT_LIGHT

  return {
    series,
    seriesAlt,

    // Flat aliases, so a `roles` array in an .mdx can name a slot as a string:
    // roles={['muted', 'series0', 'seriesAlt1']}. Same values as the arrays above.
    series0: series[0],
    series1: series[1],
    series2: series[2],
    series3: series[3],
    seriesAlt0: seriesAlt[0],
    seriesAlt1: seriesAlt[1],
    seriesAlt2: seriesAlt[2],
    seriesAlt3: seriesAlt[3],
    ...
    statMean: muted,
    statMuted: muted,
    mutedAlt: dark ? MUTED_ALT_DARK : MUTED_ALT_LIGHT,
