...
function postCard(title, subtitle) {
  // Big hero title + optional smaller muted deck. Both are supplied explicitly
  // by the caller (no render-time colon splitting); an empty subtitle renders
  // the title alone.
  const headChildren = [
    brandRow(),
    el('div', {
...
async function renderOgPng({ title, subtitle, type } = {}) {
  const clean = typeof title === 'string' ? title.trim() : ''
  const cleanSub = typeof subtitle === 'string' ? subtitle.trim() : ''
  const isPost = type === 'post' && clean
  let element
  if (isPost) {
    element = postCard(clean, cleanSub)
  } else {
