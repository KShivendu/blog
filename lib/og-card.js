...
// Deck (subtitle) sizing — a step below the title.
function subtitleFontSize(s) {
  const n = s.length
  if (n <= 40) return 44
  if (n <= 70) return 40
  if (n <= 110) return 36
...
function postCard(rawTitle) {
  // "Punchy Name: elaboration" -> big title + smaller muted deck. Split on the
  // first colon only; titles without one render as a single hero.
  const idx = rawTitle.indexOf(':')
  const hasSplit = idx > 0 && idx < rawTitle.length - 1
  const title = hasSplit ? rawTitle.slice(0, idx).trim() : rawTitle
  const subtitle = hasSplit ? rawTitle.slice(idx + 1).trim() : ''
  ...
  return frame([
    el('div', { style: { display: 'flex', flexDirection: 'column' }, children: headChildren }),
    hairline(),
  ])
}
